...
config_name = 'osp-reformulation-translate-adapted-search-hmax-general-util'
...
exp.add_algorithm('ref-adapted-search-hmax-cb', REPO, REV, ["--search", "astar(hmax(transform=osp_utility_to_cost(), use_cost_bound=true))"])
exp.add_algorithm('ref-adapted-search-hmax-nocb', REPO, REV, ["--search", "astar(hmax(transform=osp_utility_to_cost(), use_cost_bound=false))"])
...
exp.add_fetcher('data/2019-02-11-adapted-search-blind-eval')
...
exp.add_report(
    AbsoluteReport(),
    outfile='%s-all.html' % report_name)
