...
config_name = 'adapted-search-ms-1k-lrs-general-util'
config_date = '19-02-06'
...
exp.add_algorithm('ref-adapted-search-ms-lr2-1k', REPO, REV, ['--heuristic', "ms=merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_sccs(order_of_sccs=topological,merge_selector=score_based_filtering(scoring_functions=[goal_relevance,dfp,total_order(atomic_before_product=false,atomic_ts_order=reverse_level,product_ts_order=new_to_old)])),label_reduction=exact(before_shrinking=true,before_merging=false,method=TWO_TRANSITION_SYSTEMS),max_states=1000,threshold_before_merge=1,transform=osp_utility_to_cost())", '--search', "astar(ms)"])

exp.add_algorithm('ref-adapted-search-ms-lrall-1k', REPO, REV, ['--heuristic', "ms=merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_sccs(order_of_sccs=topological,merge_selector=score_based_filtering(scoring_functions=[goal_relevance,dfp,total_order(atomic_before_product=false,atomic_ts_order=reverse_level,product_ts_order=new_to_old)])),label_reduction=exact(before_shrinking=true,before_merging=false,method=ALL_TRANSITION_SYSTEMS),max_states=1000,threshold_before_merge=1,transform=osp_utility_to_cost())", '--search', "astar(ms)"])
...
exp.add_fetcher('data/2019-02-04-adapted-search-blind-eval')
