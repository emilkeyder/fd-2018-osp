...
config_name = 'ref-adapted-search-translator-ms-general-util'
config_date = '19-10-22'
...
exp.add_algorithm('ref-adapted-search-ms-lr-cb-600', REPO, REV, ['--heuristic', "ms=merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_sccs(order_of_sccs=topological,merge_selector=score_based_filtering(scoring_functions=[goal_relevance,dfp,total_order(atomic_before_product=false,atomic_ts_order=reverse_level,product_ts_order=new_to_old)])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=20000,threshold_before_merge=1,transform=osp_utility_to_cost(), use_cost_bound=true, main_loop_max_time=600)", '--search', "astar(ms)"])

#exp.add_algorithm('ref-adapted-search-ms-lr-nocb-600', REPO, REV, ['--heuristic', "ms=merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_sccs(order_of_sccs=topological,merge_selector=score_based_filtering(scoring_functions=[goal_relevance,dfp,total_order(atomic_before_product=false,atomic_ts_order=reverse_level,product_ts_order=new_to_old)])),label_reduction=exact(before_shrinking=true,before_merging=false),max_states=20000,threshold_before_merge=1,transform=osp_utility_to_cost(), use_cost_bound=false, main_loop_max_time=600)", '--search', "astar(ms)"])
