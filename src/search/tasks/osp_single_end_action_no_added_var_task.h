#ifndef TASKS_OSP_SINGLE_END_ACTION_NO_ADDED_VAR_H
#define TASKS_OSP_SINGLE_END_ACTION_NO_ADDED_VAR_H
...
class OSPSingleEndActionNoAddedVarTask : public tasks::DelegatingTask {
  protected:
    ...
    ExplicitOperator end_operator;

    const int NOT_AN_AXIOM = -1;

    std::vector<int> sg_vars_vec;

  public:
    OSPSingleEndActionNoAddedVarTask(const std::shared_ptr<AbstractTask> &parent);
    virtual ~OSPSingleEndActionNoAddedVarTask() override = default;
